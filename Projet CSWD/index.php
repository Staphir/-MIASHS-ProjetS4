...
<section id="home">
    <article class="card">
        <img src="images/main_pic_redim2.jpg" alt='Accueil' style='width:100%; height:100%'/>
        <div>
...
<aside class="top10">
    <h2 class="top10">Top 10 des histoires</h2><hr>
    <?php
    $query = "SELECT story.id, title, story.likes, user.username FROM story, user WHERE story.user_id = user.id AND published = 1 ORDER BY story.likes DESC, story.title LIMIT 10 ";
    $result = $pdo->prepare($query);
    $result->execute();
    $row = $result->fetchAll(PDO::FETCH_ASSOC);
